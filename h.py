    ...
    if strategy_performance is not None and benchmark_performance is not None and not strategy_performance.empty and not benchmark_performance.empty:
        ...
        if use_compound:
            ...
            growth_df = pd.DataFrame({
                '日期': dates,
                '策略组合资产': strategy_total_capital,
                f'{benchmark_stock} 资产': benchmark_total_capital
            })

            # 格式化数据表，去除小数点
            growth_df_display = growth_df.copy()
            growth_df_display['策略组合资产'] = growth_df_display['策略组合资产'].round(0).astype(int)
            growth_df_display[f'{benchmark_stock} 资产'] = growth_df_display[f'{benchmark_stock} 资产'].round(0).astype(int)

            # 显示资产增长数据表
            st.subheader("资产增长数据表")
            ...
            fig2.add_trace(go.Bar(
                x=growth_df['日期'],
                y=growth_df[f'{benchmark_stock} 资产'],
                name=f'{benchmark_stock} 资产',
                marker_color='green'
            ))

            fig2.update_layout(
                barmode='group',  # 将柱状图设置为分组模式
                title='资产增长图',
                xaxis_title='日期',
                yaxis_title='资产总额（元）',
                hovermode='x unified',
            ...
            result_df = pd.DataFrame({
                ...
                '预期资产（元）': [f"{strategy_total_capital[-1]:,.0f}", f"{benchmark_total_capital[-1]:,.0f}"]
            })
