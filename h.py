    ...
    if strategy_performance is not None and benchmark_performance is not None and not strategy_performance.empty and not benchmark_performance.empty:
        ...
        if use_compound:
            ...
            growth_df = pd.DataFrame({
                '日期': dates,
                '策略組合總資產': strategy_total_capital,
                '策略組合本金': strategy_principal,
                '策略組合收益': strategy_interest,
                f'{benchmark_stock} 總資產': benchmark_total_capital,
            ...
            cols_to_round = ['策略組合總資產', '策略組合本金', '策略組合收益', f'{benchmark_stock} 總資產', f'{benchmark_stock} 本金', f'{benchmark_stock} 收益']
